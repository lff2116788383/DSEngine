...
TEST_CASE("Given_ReferenceDemo157StartupScene_When_RuntimeStarts_Then_MaterialShowcaseSceneLoadsWithoutKnownMissingResources", "[engine][smoke][cpp_runtime][startup_scene][reference_demo]") {
    const std::string repoRoot = ResolveRepoRoot();
    const std::string scenePath = "assets/scenes/reference_demo_15_7.scene.json";

    const std::string output = RunSampleAndCapture(repoRoot, scenePath);
    INFO(output);
    REQUIRE(output.find("startup_scene_env=assets/scenes/reference_demo_15_7.scene.json") != std::string::npos);
    REQUIRE(output.find("startup_scene_loaded path=assets/scenes/reference_demo_15_7.scene.json") != std::string::npos);
