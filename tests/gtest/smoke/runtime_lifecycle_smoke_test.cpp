...
TEST_F(RuntimeLifecycleSmokeTest, EngineRunConfig默认值) {
...
TEST_F(RuntimeLifecycleSmokeTest, RuntimeContext模式切换) {
    RuntimeContext ctx;
...
TEST_F(RuntimeLifecycleSmokeTest, BusinessRuntimeBridge无World时Cpp模式引导失败) {
    RuntimeContext ctx;
    ctx.business_mode = BusinessMode::Cpp;
    ctx.world = nullptr;
...
TEST_F(RuntimeLifecycleSmokeTest, FramePipeline未初始化安全操作) {
...
TEST_F(RuntimeLifecycleSmokeTest, 核心服务快速启停) {
...
TEST_F(RuntimeLifecycleSmokeTest, RuntimeServices注入与默认) {
...
TEST_F(RuntimeLifecycleSmokeTest, 多次FramePipeline构造析构不崩溃) {
    for (int i = 0; i < 3; ++i) {
        FramePipeline pipeline;
...
TEST_F(RuntimeLifecycleSmokeTest, BusinessRuntimeBridgeLua模式无脚本引导失败) {
    RuntimeContext ctx;
