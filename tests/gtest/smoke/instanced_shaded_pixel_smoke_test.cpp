/**
 * @file instanced_shaded_pixel_smoke_test.cpp
 * @brief Final-Feat-3 硬件实例化 + 高级 shading 组合「活体」像素冒烟 —— 用高层
 *        MeshRenderer::DrawInstancedShaded（消费内建程序 BuiltinProgram::ForwardInstancedShaded：
 *        forward_shaded_instanced.vert 按 gl_InstanceIndex 取每实例 model 矩阵 SSBO\@slot 0[set7.b0]
 *        后施 model + vp，复用高级 shading frag forward_shaded.frag —— shading_mode 0/2-6 +
 *        clustered 点光 + GI 等）。
 *
 * 设计要点（参 skinned_shaded smoke，验证「每实例独立 model 矩阵」+ 高级 shading）：
 *  - 单块居中于原点的窄竖直矩形面片（x∈[-0.2,0.2], y∈[-0.7,0.7]），法线 +X（朝光源）。
 *  - 两个实例 model：实例 0 = 平移(-0.5,0,0)、实例 1 = 平移(+0.5,0,0)。
 *    实例化正确 → 左面片落到 x∈[-0.7,-0.3]（中心 x=64px）、右面片落到 x∈[0.3,0.7]（中心 x=192px），
 *    两者皆朝光被照亮，中缝 x=128 与四角为清屏黑。
 *  - 若实例化失效（gl_InstanceIndex 未取到各自 model，或仅广播实例 0）→ 右侧 x=192 不出现面片，
 *    断言失败；若高级 shading frag 未生效 → 朝光面片不亮，亦失败。
...
RenderTargetReadback RenderInstancedShaded(RhiDevice& device, int shading_mode) {
    ...
    // 两个实例：平移把居中面片搬到左右目标位置（每实例独立 model 矩阵）。
    ...
    if (cmd) {
        ...
        renderer.DrawInstancedShaded(*cmd, device, verts, indices, models, I, proj, cam_pos,
...
RenderTargetReadback RenderPbr(RhiDevice& device) { return RenderInstancedShaded(device, 0); }
RenderTargetReadback RenderHalfLambert(RhiDevice& device) { return RenderInstancedShaded(device, 3); }

// 左右两面片皆亮、中缝与四角清屏黑 —— 证明每实例独立 model 矩阵 + 高级 shading 同时生效。
void VerifyTwoLitInstances(const RenderTargetReadback& rb, const char* backend) {
    ...
    // 实例 0（左）与实例 1（右）皆出现且被照亮 → 每实例 model 矩阵各自生效。
...
// ============================================================
// 三后端单独像素验证（活体消费 MeshRenderer::DrawInstancedShaded + ForwardInstancedShaded）。
// ============================================================

// ── PBR(0)：每实例位移 + 标准 PBR shading ──
TEST(InstancedShadedPixelSmokeTest, OpenGLPbr) { RunSingleBackend(dse::test::RunOpenGL(RenderPbr), "OpenGL"); }
TEST(InstancedShadedPixelSmokeTest, D3D11Pbr) { RunSingleBackend(dse::test::RunD3D11(RenderPbr), "D3D11"); }
TEST(InstancedShadedPixelSmokeTest, VulkanPbr) { RunSingleBackend(dse::test::RunVulkan(RenderPbr), "Vulkan"); }
TEST(InstancedShadedPixelSmokeTest, PbrCrossBackend) { CheckCrossBackend(RenderPbr, 12.0); }

// ── HalfLambert-Static(3)：每实例位移 + 高级 shading 分支 ──
TEST(InstancedShadedPixelSmokeTest, OpenGLHalfLambert) { RunSingleBackend(dse::test::RunOpenGL(RenderHalfLambert), "OpenGL"); }
TEST(InstancedShadedPixelSmokeTest, D3D11HalfLambert) { RunSingleBackend(dse::test::RunD3D11(RenderHalfLambert), "D3D11"); }
TEST(InstancedShadedPixelSmokeTest, VulkanHalfLambert) { RunSingleBackend(dse::test::RunVulkan(RenderHalfLambert), "Vulkan"); }
TEST(InstancedShadedPixelSmokeTest, HalfLambertCrossBackend) { CheckCrossBackend(RenderHalfLambert, 12.0); }
