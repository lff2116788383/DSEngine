...
TEST_F(JobSystemEventBusIntegrationTest, 异步任务完成后发布事件) {
...
TEST_F(JobSystemEventBusIntegrationTest, 多个异步任务并行完成后各自发布事件) {
...
TEST_F(JobSystemEventBusIntegrationTest, 依赖链任务完成后发布事件) {
...
TEST_F(JobSystemEventBusIntegrationTest, 事件回调中提交新任务) {
...
TEST_F(JobSystemEventBusIntegrationTest, 多线程并发发布事件不崩溃) {
...
TEST_F(JobSystemEventBusIntegrationTest, 通过ServiceLocator获取服务并协作) {
    auto* js = ServiceLocator::Instance().Get<JobSystem>();
...
TEST_F(JobSystemEventBusIntegrationTest, 高优先级任务先完成并发布事件) {
