...
TEST_F(AssetAsyncFullChainIntegrationTest, 异步加载后PumpMainThreadCallbacks调度回调) {
...
TEST_F(AssetAsyncFullChainIntegrationTest, PendingMainThreadCallbacks初始为零且非负) {
    // 初始应为 0 或接近 0
...
TEST_F(AssetAsyncFullChainIntegrationTest, 高水位记录峰值) {
    // 高水位应 >= 0
    std::size_t hw = asset_manager->PendingMainThreadCallbacksHighWatermark();
    EXPECT_GE(hw, 0u);
}

TEST_F(AssetAsyncFullChainIntegrationTest, 异步加载与JobSystem协作) {
...
TEST_F(AssetAsyncFullChainIntegrationTest, 异步加载与EventBus协作) {
...
TEST_F(AssetAsyncFullChainIntegrationTest, 多次PumpMainThreadCallbacks不崩溃) {
    for (int i = 0; i < 100; ++i) {
...
TEST_F(AssetAsyncFullChainIntegrationTest, 无效路径异步加载回调不崩溃) {
