...
TEST_F(RuntimeSubsystemIntegrationTest, FramePipeline与Physics2D手动装配后物理更新) {
    World world;
...
TEST_F(RuntimeSubsystemIntegrationTest, EngineInstance构造析构不泄漏) {
...
TEST_F(RuntimeSubsystemIntegrationTest, EngineInstance多次构造析构稳定) {
    for (int i = 0; i < 3; ++i) {
        EngineRunConfig config;
        EngineInstance instance(config);
        EXPECT_NE(instance.pipeline(), nullptr);
...
TEST_F(RuntimeSubsystemIntegrationTest, ServiceLocator注册Physics2D与EventBus) {
...
TEST_F(RuntimeSubsystemIntegrationTest, BusinessRuntimeBridgeLua模式创建实体) {
...
TEST_F(RuntimeSubsystemIntegrationTest, 服务全链注册使用注销再注册) {
...
TEST_F(RuntimeSubsystemIntegrationTest, Physics2DSystemShutdown后状态重置) {
    World world;
    auto e = world.CreateEntity();
