...
TEST(ModuleTest, OnInit被调用并接收参数) {
...
TEST(ModuleTest, OnUpdate被调用并传递delta_time) {
    MockModule mod("TestModule");
    World world;
...
TEST(ModuleTest, OnFixedUpdate被调用并传递fixed_delta) {
    MockModule mod("TestModule");
    World world;
...
TEST(ModuleTest, 多次Update调用计数正确) {
