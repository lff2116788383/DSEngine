...
TEST(BoundingBoxComponentTest, 默认包围盒center在原点) {
...
TEST(BoundingBoxComponentTest, 默认包围盒extents为零) {
    BoundingBoxComponent bb;
...
TEST(BoundingBoxComponentTest, 对称包围盒center在原点) {
    BoundingBoxComponent bb;
...
TEST(BoundingBoxComponentTest, 对称包围盒extents正确) {
    BoundingBoxComponent bb;
    bb.min_extents = glm::vec3(-1.0f, -2.0f, -3.0f);
    bb.max_extents = glm::vec3(1.0f, 2.0f, 3.0f);
...
TEST(BoundingBoxComponentTest, 非对称包围盒center偏移) {
    BoundingBoxComponent bb;
...
TEST(BoundingBoxComponentTest, 非对称包围盒extents正确) {
    BoundingBoxComponent bb;
    bb.min_extents = glm::vec3(0.0f, 0.0f, 0.0f);
    bb.max_extents = glm::vec3(2.0f, 4.0f, 6.0f);
...
TEST(BoundingBoxComponentTest, 负值包围盒) {
    BoundingBoxComponent bb;
...
TEST(Components3DPhysicsTest, RigidBody3DType枚举值) {
...
TEST(Components3DPhysicsTest, RigidBody3D默认值) {
...
TEST(Components3DPhysicsTest, BoxCollider3D默认值) {
...
TEST(Components3DPhysicsTest, SphereCollider3D默认值) {
...
TEST(Components3DPhysicsTest, MeshCollider3D默认值) {
...
TEST(Components3DTest, Camera3D默认值) {
...
TEST(Components3DTest, DirectionalLight3D默认值) {
...
TEST(Components3DTest, PointLight默认值) {
...
TEST(Components3DTest, SpotLight默认值) {
...
TEST(Components3DTest, SteeringComponent默认值) {
