...
void RigidActor::AttachShape(PxShape* shape) {
    if (shape == nullptr || px_rigid_actor_ == nullptr) return;
    px_rigid_actor_->attachShape(*shape);
    shape->release(); // Transfer ownership to actor
}

void RigidActor::DetachShape(PxShape* shape) {
    if (shape == nullptr || px_rigid_actor_ == nullptr) return;
    px_rigid_actor_->detachShape(*shape);
}