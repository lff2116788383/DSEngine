...
layout(std140, set = 2, binding = 0) uniform PerMaterial {
    ...
    vec4 _mat_warm_color;
    vec4 _mat_cool_color;
    float _mat_ramp_smoothness;
    float _mat_ramp_bands;
};
layout(set = 2, binding = 1) uniform sampler2D albedo_tex;
#define albedo_color _mat_albedo_color
#define warm_color _mat_warm_color
#define cool_color _mat_cool_color
#define ramp_smoothness _mat_ramp_smoothness
#define ramp_bands _mat_ramp_bands
...
void main() {
    ...
    // ===== 用户 surface() 代码 =====
    {
        vec4 tex = dssl_sample(albedo_tex, UV);
