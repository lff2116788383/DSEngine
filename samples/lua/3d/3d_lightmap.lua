...
function M.Setup(config)
    ...
    pcall(ecs.add_lightmap,floor, 256, 256) -- lightmap UV resolution
    ...
    for _, w in ipairs(walls) do
        ...
        pcall(ecs.add_lightmap,e, 128, 128)
    end

    -- Ceiling
    local ceil = ecs.create_entity()
    ecs.add_transform(ceil, 0, 6, 0, 12, 0.2, 12)
    ecs.add_mesh_renderer(ceil, 0.6, 0.6, 0.6, 1.0, cube_vertices(), cube_indices())
    pcall(ecs.add_lightmap,ceil, 256, 256)

    -- Interior objects (receive baked light)
    local table_e = ecs.create_entity()
    ecs.add_transform(table_e, 0, 0.8, 0, 3, 0.1, 2)
    ecs.add_mesh_renderer(table_e, 0.6, 0.6, 0.6, 1.0, cube_vertices(), cube_indices())
    pcall(ecs.add_lightmap,table_e, 64, 64)

    -- Light sources for baking
    local light1 = ecs.create_entity()
    ecs.add_transform(light1, -2, 5, 0, 1, 1, 1)
    ecs.add_point_light_3d(light1, 1.0, 0.9, 0.7, 3.0, 10.0)

    local light2 = ecs.create_entity()
    ecs.add_transform(light2, 2, 5, 0, 1, 1, 1)
    ecs.add_point_light_3d(light2, 0.7, 0.9, 1.0, 2.0, 10.0)
end
