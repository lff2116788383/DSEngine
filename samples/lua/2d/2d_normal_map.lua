...
function M.Setup(config)
    ...
    -- Set dim ambient
    dse.ecs.set_ambient_2d(0.05, 0.05, 0.08)

    -- Point light that will orbit
    local light = dse.ecs.create_entity()
    dse.ecs.add_transform(light, 2.0, 2.0, 0, 1, 1, 1)
    dse.ecs.add_light_2d(light)
    dse.ecs.set_light_2d_color(light, 1.0, 0.9, 0.7)
    dse.ecs.set_light_2d_intensity(light, 3.0)
    dse.ecs.set_light_2d_range(light, 8.0)
    state.light = light
    ...
    dse.ecs.add_normal_map_2d(surface, tex) -- uses white as placeholder normal
    ...
    dse.ecs.add_normal_map_2d(obj1, tex)
    ...
    dse.ecs.add_normal_map_2d(obj2, tex)
end
