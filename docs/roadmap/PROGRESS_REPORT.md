...
音频系统              ████████████ 70%  ✅ BGM/SFX+3D空间+DSP混音总线+效果链+Lua API
跨平台                ████████████ 15%  ❌ 仅 Windows
网络                  ████████████  0%  ❌ 完全缺失
```
...
| 提交 | 功能 | 复杂度 |
|:----:|------|:------:|
| `latest` | 音频 DSP 混音总线 + 效果链（AudioBus/DspEffect/Lua API） | 🟡 中 |
...
| **实时全局光照** | ❌ 无（有Probe IB+SSAO） | ✅ HDRP GI | ✅ Lumen |
...
| **无 GPU Driven 渲染** | 🟡 严重 | 大规模场景下有 CPU 瓶颈，无法支撑万人同屏/开放世界级别 |
| **无实时全局光照** | 🟡 严重 | 光照精度不如 Lumen/HDRP GI，氛围感有差距 |
| **无网络模块** | 🟡 严重 | 无法做联机游戏 |
| **无资源流式加载** | 🟡 严重 | 无法支撑大世界/无缝地图 |
| **音频系统** | 🟢 中等 | BGM/SFX + 3D 空间 + DSP 混音总线已实现，待 biquad/delay 节点图实施 |
| **风格化渲染** | 🟢 中等 | Toon/Cel+Banding 已完成，待 Outline/Edge Detection 深化 |
| **编辑器体验** | 🟢 中等 | ImGui 实现功能完整，但 UI/UX 不如原生 GUI 引擎 |
| **无 AI/导航** | 🟢 中等 | 无 NavMesh/寻路，需要自行实现 |
...
```
Phase 1 — 基础能力补全 (~1周)
  ├── 音频 DSP 节点图实施（ma_lpf_node/ma_biquad_node/ma_delay_node）  [1-2天]
  ├── Compute Shader 基础管线（三后端 Dispatch 抽象）                    [2-3天]
  └── NavMesh / 寻路（集成 Recast/Detour + NavMeshAgent + Lua API）     [3-4天]
...
Phase 4 — 光照升级
  └── 实时 GI（DDGI Irradiance Probe + Raymarching 更新 + 采样插值）    [7-10天]
...
>
> **与 UE5/Unity 的差距不在架构设计（ECS/JobSystem/RenderGraph 与行业最佳实践一致），而在于功能覆盖深度——缺少 GPU Driven 渲染、实时 GI、网络模块、跨平台——这些需要成千上万倍于当前的人年投入。与 Godot 相比，DSE 在 ECS 架构和 3D 物理上占优，但在跨平台、编辑器体验和社区生态上全面落败。**
>
