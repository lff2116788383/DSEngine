...
> 生成日期：2026-05-18（基于代码统计自动更新）
> 数据来源：代码库直接统计（powershell 行数统计 + grep 验证） + git 提交历史
...
| 维度 | 数值 |
|------|:----:|
| **引擎核心 C++** | ~73,251 行（engine/ 62,346 + modules/ 10,905） |
| **编辑器 C++** | ~9,995 行（apps/editor_cpp/） |
| **测试代码** | ~23,610 行 |
| **Lua 脚本** | ~11,100 行（samples/ 8,547 + examples/ 2,552） |
| **自有代码合计** | ~118,000 行 |
| **第三方依赖** | 38 个（~200K+ 行） |
| **渲染后端** | OpenGL / Vulkan / D3D11（基础渲染对等；Compute 功能仅 GL 完整，VK/DX11 待补） |
| **测试体系** | ~1,675 个测试（单元 + 集成 + 冒烟） |
...
渲染 Vulkan 后端       █████████░░░ 75%  ⚠️ 基础渲染完整；DDGI/TressFX/Grass Compute 未移植（GLSL 430 硬编码）
渲染 D3D11 后端        █████████░░░ 75%  ⚠️ 基础渲染完整；DDGI/TressFX/Grass Compute 未移植（需 HLSL CS）
动画系统              ████████████ 82%  ✅ IK/Layering/2DBlend/FSM
场景管理              ████████████ 85%  ✅ SubScene/Prefab/异步加载
资产管理              ████████████ 85%  ✅ 异步+LRU+热重载+PAK打包
编辑器                ████████████ 80%  ✅ 28个面板，功能完整
Lua 脚本绑定          ████████████ 90%  ✅ 绑定全面，340+ API，文档全覆盖
测试体系              ████████████ 90%  ✅ 147个测试文件，1,675个测试用例
实时全局光照          ████████░░░░ 70%  ⚠️ DDGI GL 后端完整；VK/DX11 Compute Shader 为 GLSL 430 硬编码，无法在非 GL 后端运行
...
| 提交 | 功能 | 复杂度 |
|:----:|------|:------:|
| `latest` | DDGI 实时全局光照全流程（Step A-E：RSM MRT + Compute 探针更新 + PBR 辐照度采样 + Lua API + 全量审查修复） | 🔴 高 |
| — | TressFX 毛发系统（Strand-based 物理 Compute 模拟 + Kajiya-Kay 光照 + SSBO 渲染 + LOD + 槽位回收 + Lua API）【GL 完整实现；VK/DX11 `DrawHairStrands` 为空 stub】 | 🔴 高 |
...
| **实时全局光照** | ⚠️ DDGI Probe（仅 GL 后端可用；VK/DX11 Compute 未移植） | ✅ HDRP GI | ✅ Lumen |
...
| **代码可读性** | ✅ ~11.8 万行可通读全部源码 | 150 万行难掌握全貌 | **DSE** |
| **渲染能力** | PBR/后处理/阴影 | PBR/后处理/阴影 | **平手** |
| **2D 能力** | 完整 | 行业最佳之一 | **Godot 略胜** |

### 2.3 核心结论

> **DSEngine 在功能覆盖面 ≈ Godot 4.x**（都有 2D→3D→编辑器→物理→脚本的完整管线），但代码量仅为 Godot 的 **~1/13**。
>
> 在**“架构/体量比”**这个独特维度上，DSE 找不到对手——用一个人能通读的 ~11.8 万行代码，实现了需要数百万行才能做到的引擎完整度。
...
| **三端渲染** | OpenGL / Vulkan / D3D11 基础渲染对等（延迟渲染/后处理/阴影/Instancing/LOD）；Compute 功能（DDGI/TressFX/Grass）仅 GL 完整，VK/DX11 待移植 |
| **代码量极小** | ~11.8 万行自研代码（含 Lua），个人可在数周内通读全部源码——这是 UE/Unity 数千万行完全做不到的 |
| **工程效率极高** | 用 ~1/13 的 Godot 代码量实现接近的功能广度 |
...
| **实时全局光照** | ⚠️ 部分完成 | DDGI Probe GL 后端全流程可用（RSM → Compute → PBR 采样）；**VK/DX11 无法运行**（Compute Shader 为 GLSL 430 硬编码，VK 需 SPIR-V、DX11 需 HLSL）；Lua API 已暴露 |
| **无网络模块** | 🟡 严重 | 无法做联机游戏 |
| **~~无资源流式加载~~** | ✅ 已完成 | StreamingManager：Zone 距离触发 + 异步 IO 优先级队列 + 每帧预算 + 资源引用持有 + Lua API 12 函数 |
...
| **2** | **~~SSBO → UBO fallback~~** | ✅ 已完成 | — | `gl_shader_manager.cpp` 已有 `TransformSSBOToUBO` 动态降级，`light_buffer.cpp` 根据 `SupportsSSBO()` 自动切换；GL 3.3 下 `kMaxUBOLights=64` |
...
| **9** | **SDK 文档 / API 参考** | ✅ 已完成 | — | `docs/api/LUA_API.md`（340+ API）+ `docs/api/CPP_API.md`（10 大类 21 个头文件）+ `docs/api/API_GAP_ANALYSIS.md` |
...
Phase 3 — 视觉特性 (~3周)
  ...
  └── ✅ 毛发系统（TressFX Strand 物理 Compute 模拟 + Kajiya-Kay 渲染 + LOD + GL 完整实现）
       ⚠️ VK/DX11 DrawHairStrands 为空 stub，Compute 物理未移植
...
Phase 4 — 光照升级
  ...
  └── ✅ 实时 GI Step E — visibility 越界守卫 + 公共光源相机提取 + 全量审查修复
       ⚠️ VK/DX11 DDGI Compute 未移植（shader 为 GLSL 430 硬编码，VK 需 SPIR-V、DX11 需 HLSL）

Phase 1-4 GL 后端全部完成 ✅ — 实际用时约 5 周
⚠️ VK/DX11 Compute 功能债务：DDGI + TressFX + Grass 三组 Compute Shader 待移植
...
Phase 5 — 已知技术债务
  ...
  └── ⬜ 跨平台抽象层（engine/ 13 文件 25 处 Win32 硬编码，modules/ 0 处）
```
...
> **DSEngine 当前的自研代码量约 11.8 万行（C++ + Lua），功能覆盖 2D→3D→编辑器→物理→脚本→三端渲染→实时 GI→资源流式加载的完整管线，在"个人/小团队自研引擎"这个维度里完成度是顶级水准。**
>
...
>
> **当前最大技术债务：① VK/DX11 Compute 移植（DDGI/TressFX/Grass 三组 shader）；② engine/ → modules/ 架构依赖违规（3 文件 17 处）；③ 跨平台抽象层（engine/ 13 文件 25 处 Win32 硬编码）。**
>
