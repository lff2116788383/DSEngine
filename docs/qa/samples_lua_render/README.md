...
| A3 | `dssl/demo_npr_light` | `samples/lua/dssl/demo_npr_light.lua:5` | `module 'dse.ecs' not found`（用了 `require("dse.ecs")`） | 启动 load 失败，FramePipeline 初始化失败，**完全跑不起来** | `dse.ecs` 是 C++ 注入全局表，非 Lua 模块；改为 `local ecs = dse.ecs`（参照 `dssl_material_demo.lua:7`） |
| A4 | `dssl/demo_toon` | `samples/lua/dssl/demo_toon.lua:5` | `attempt to call a nil value (field 'get_world')` | 启动 load 失败，**完全跑不起来** | `dse.get_world()`/`dse.get_scene()` 为已失效 API，按现行 `dse.ecs` API 重写实体创建 |
