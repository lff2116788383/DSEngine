...
渲染 OpenGL 后端      ████████████ 93%  ✅ PBR+后处理+阴影+延迟渲染+WBOIT+Water
渲染 Vulkan 后端       ████████████ 87%  ✅ 真实实现，默认关闭
渲染 D3D11 后端        ████████████ 87%  ✅ 完整实现
...
测试体系              ████████████ 85%  ✅ 100个测试文件
风格化渲染            ████████████ 35%  ✅ Toon/Cel+Banding，待Outline
音频系统              ████████████ 55%  ✅ 基础播放，缺DSP混音
跨平台                ████████████ 15%  ❌ 仅 Windows
网络                  ████████████  0%  ❌ 完全缺失
```
...
| 提交 | 功能 | 复杂度 |
|:----:|------|:------:|
| `latest` | Water/Ocean 系统三后端（Gerstner 波浪 + 折射/反射 + Lua API） | 🔴 高 |
...
| **PBR 渲染** | ✅ 金属/粗糙度工作流 | ✅ | ✅ |
| **后处理链** | ✅ Bloom/SSAO/TAA/FXAA/DOF/MotionBlur/SSR/ACES | ✅ | ✅ |
...
| **音频较基础** | 🟢 中等 | 仅基础播放 + 3D 空间化，缺混音总线和 DSP 效果链 |
...
```
v0.1.0-alpha (1周后)  ← 当前目标
  ├── 完整 2D + 3D SDK 打包
  ├── 消费者示例可编译运行
  └── 最小 GPU 要求 GL 4.3+

v0.2.0 (3-4周后)
  ├── SSBO→UBO fallback → GL 3.3+
  ├── .dds / BCn 纹理直接上传
  ├── 消费者示例完善
  └── Release 二进制 + LTCG
