...
### 4.2b `FrameUpdateContext`（engine/base/frame_update_context.h，纯数据）
贯穿 update graph 的**统一帧上下文通道**：把"需要穿过整条 update graph 的逐帧状态"聚合到
单一可扩展载体。`TimeContext` 作为其 `time` 子成员；未来新增贯穿式逐帧状态（插值 alpha、
暂停标志、关卡时间等）只需在此结构体加字段，**无需再逐层改 `Update`/`OnUpdate` 签名**——
这是 time_scale 落地时暴露的"贯穿式特性要改多层签名"成本的长期解法。
```cpp
...
```
> 与 `dse::render::FrameContext`（相机矩阵，渲染期载体）区分：本结构服务逻辑更新期。
...
- `EngineInstance::Tick()`：先对**真实** dt 做首帧/卡顿钳制，再乘 scale 得 `scaled`；
  `accumulator_ += scaled`（`scale=0` → 不累加 → 物理冻结）；构建 `FrameUpdateContext{TimeContext{...}, frame_index++}` 传入 `pipeline_->Update`。
- `FramePipeline::Update(const FrameUpdateContext&)` → `RunFrameUpdate` → `RunRuntimeUpdateGraph(const FrameUpdateContext&)`。
- `IBuiltinModules::UpdateGameplay2D/3D(World&, const FrameUpdateContext&)`、`UpdateFallback3D`（引擎内部接口）。
- 模块内部经 `frame.time` 取出 `TimeContext`，再按通道分流。
- 外部插件 `IModule::OnUpdate(world, frame.time.scaled_dt)`：保持 `float` 签名，传入 scaled（文档约定）。
- gameplay 2D/3D 模块：UI 子系统传 `frame.time.unscaled_dt`（暂停菜单不冻结），其余 gameplay 子系统传 `frame.time.scaled_dt`。
...
| `engine/base/frame_update_context.h`（新） | `FrameUpdateContext`（统一帧上下文通道：`TimeContext time` + `frame_index`） |
| `engine/runtime/frame_pipeline.{h,cpp}` | `Update(const FrameUpdateContext&)` |
| `engine/runtime/runtime_frame_ops.*` / `runtime_update_graph.*` | 透传 FrameUpdateContext |
| `engine/runtime/i_builtin_modules.h` + `modules/runtime_bridge/builtin_modules_impl.*` | UpdateGameplay2D/3D 收 FrameUpdateContext |
