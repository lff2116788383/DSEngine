...
| GPU-driven 剔除 / Hi-Z | ✅ | ✅ | ❌（门控 indirect-draw，仍裁剪） | ❌ |
...
但 GPU-driven/Hi-Z 仍门控 `SupportsIndirectDraw()`、skinning/grass 仍门控 `SupportsSSBOCompute()`
（均仍 false）→ demo default 管线视觉不变（golden 未动）。**翻转无需改裁剪代码**，
