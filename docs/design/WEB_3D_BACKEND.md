...
- [~] 文档：Web 能力矩阵（§B4 ✅）、限制（§5 ✅）、部署指南（§7 ✅）；多线程部署头随该里程碑落地
...
### 7.4 跨源隔离（多线程里程碑前置）
当前单线程产物**无需**特殊响应头。一旦启用多线程（SharedArrayBuffer + `-pthread`，见 §3 里程碑），
托管端必须下发跨源隔离头使 `SharedArrayBuffer` 可用：
...
其下所有跨源子资源需带 `Cross-Origin-Resource-Policy`/CORP 或经 CORS，否则被拦截。该头不影响单线程产物，可提前配置。
