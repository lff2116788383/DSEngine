...
| **B5** ✅ | CI：WebGPU headless 渲染回归已接入 `.github/workflows/ci.yml` 的 `build-web` job——host 编译 `dse_shader_compiler` → 生成 embed headers → emsdk 3.1.64 配置+构建 `dse_web_host` → 安装 headless Chrome 系统库 + harness 依赖 → 跑 **双后端**视觉回归（`--backend webgl2 --name web3d`(SwiftShader/ANGLE) + `--backend webgpu --name web3d_wgpu`(Dawn+SwiftShader Vulkan)），对照已提交 golden 超阈值即 fail，actual/diff PNG 作 artifact 上传。`workflow_dispatch` 已开（CI 无额度时手动触发）。注：受 runner 额度限制，build-web job 的首次真实云端运行待额度恢复；命令对齐本地多次验证（WebGPU 0.118% / WebGL2 0.045% < 2%） |
...
| `SupportsSSBOCompute()` | ✅ | ✅ | ✅（Task 5 翻转：skinning WGSL 已接入；GPU→GPU 数据流不需 CPU 同步回读；系统休眠属能力声明） | ❌ |
...
（如 CustomLiteLua）激活并验证双后端；Task 5 起 `SupportsSSBOCompute()=true`：skinning 已接 WGSL（消费方系统休眠，属能力声明），grass/hair 未传 WGSL → 句柄 0 优雅 CPU 回退。
...
- [x] Web headless 视觉回归 harness（A4，swiftshader + 帧计数 + rAF 抓帧 + 非平凡守护 + golden diff）；CI 双后端接入（B5，`ci.yml::build-web`）✅
- [x] 运行期能力探测自动选路径 + URL 覆盖（?mode/?profile/?lua）+ 加载进度 + 大资产 MEMFS→fetch 懒加载（A5）
- [x] WebGPU 后端工具链 + 设备/交换链骨架 + 每帧 clear pass（B0）
- [x] WebGPU 后端 + compute/SSBO 全链路（B1–B2 ✅、B3a compute 基础设施+WGSL 自检 ✅、B3b 逐特性手译 WGSL 自检 ✅、Task 4/5 翻 `SupportsIndirectDraw`/`SupportsSSBOCompute` ✅；GPU skinning WGSL 已接入但消费方系统休眠，属能力声明、非可见激活——见 B3 行 Task 5 bullet）
