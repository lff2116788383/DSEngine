...
17. ~~`dse-graph` 1 条~~ —— **已修**（2/2，连续 3 次运行坐标完全一致）。改法：**不再经由右键菜单建节点**，
    直接拖默认图里既有的两个节点 —— `node2 «Color».Color`（Color 输出）→ `node0 «PBR Output».Emission`
    （Color 输入），类型一致且两节点都在画布可视区内。建节点会让画布**逐帧缓动取景**（可视区与坐标
    持续变化），正是这条用例长期不稳定的主因。配套：`ShaderGraphPinScreenPos` 复用面板自己的引脚
    布局公式；拖动前轮询到"引脚屏幕坐标连续 6 帧不变"；并断言两个引脚都在画布可视区内。
    产品侧真缺陷（画布 hover 用 `IsItemHovered()`，按住期间恒 false → release 分支永不执行）已在
    上一提交修复。
18. **`dse-terrain` 抖动**：进一步定位后的新证据（比第 9 条更硬）——
    - 面板侧已给四个笔刷按钮加稳定 `###` ID（`###terrain_brush_lower` 等），测试按 ID 点击；
    - 测试侧试过：等条目矩形连续 4 帧不变、手动「移动→按下→等帧→松开」替代 `ItemClick`、
      以及**同一运行内最多重试 8 次**（每次重试前重新展开/滚顶/聚焦，且用 `NoError` 查询避免
      产生引擎 Error）——**命中率仍恰好 50%，说明失败运行里按钮整轮都不触发**，不是偶发落空；
    - 面板内埋点显示：失败运行与通过运行**都**把该按钮置为 ActiveId（`0xA62A0B28`），
      窗口 `win_hovered=1`、`focused=1`，只是松开时回调没触发。
    结论：差异在**运行级别**（测试引擎的输入/焦点路由），不是 Terrain Brush 面板的产品逻辑——
    50% 通过的运行恰恰证明按钮与状态更新链路本身是好的。该用例保留为**已知抖动项**，
    断言不放水；若要彻底消除，需从引擎侧查输入投递（例如 release 帧的路由/ActiveId 归零时机）。

当前分组统计：**30 绿 / 0 红**（基线 7/24；`dse-terrain` 约 50% 抖动时表现为 29/1）。
