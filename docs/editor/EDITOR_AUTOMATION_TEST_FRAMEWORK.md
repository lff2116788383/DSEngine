...
> **v1.1 校准说明（已对照代码核实）**：
> 1. **可执行名**：编辑器产物是 `bin/dsengine-editor.exe`（CMake target `dse_editor_cpp`，`OUTPUT_NAME "dsengine-editor"`），全文已统一（旧稿误写 `dsengine_editor_cpp.exe`）。
> 2. **构建**：本仓走 CMake preset，构建目录是 `out/build/windows-x64-debug`（非 `build_vs2022`），§8 CI 已改。
> 3. **工具命名**：编辑器已注册 **32 个 `dsengine_*` Tool**（`apps/editor_cpp/src/editor_control_tools.cpp`）。用例 DSL **直接复用现有工具名**，不再用点分名（旧稿 `app.ping`/`project.open`/`editor.screenshot` 等均不存在）；`expect` 字段已按真实 result schema 对齐。真缺的仅 `dsengine_project_open` / `dsengine_editor_quit` 两个 P0 工具（见 §5.2）。
> 4. **CI 的 GPU 现实**：L1+（启动编辑器 + 截图）需真实 GPU，GitHub 托管 runner 无 GPU，必须自托管 GPU runner（见 §8.1，呼应 RHI D12）。
...
本框架围绕 DSEngine 编辑器（`dsengine-editor.exe`）构建，覆盖四种测试场景：
...
在 suite 运行前和运行后，主动扫描并清理残留的编辑器进程（参考工业框架中 `Get-Process dsengine-editor | Stop-Process` 的做法）。
...
│                    │             │  ┌─ dsengine_ping              │
│                    │             │  ├─ dsengine_project_open          │
│                    │             │  ├─ dsengine_editor_screenshot     │
│                    │             │  └─ dsengine_editor_quit              │
└───────────────────┘             └──────────────────────────┘
```
...
command:
  ...
  calls:
    - method: dsengine_ping
      expect:
        result.pong: true                 # 现有工具仅返回 {"pong": true}

    - method: dsengine_project_open          # 待新增（见 §5.2）；现有仅有 scene 级 load
      params:
    ...
    - method: dsengine_scene_get_state
      expect:
        result.entity_count: 4            # 现有字段：entities[] / entity_count / editor_state
        result.editor_state: "edit"

    - method: dsengine_entity_create
      params:
        name: "Enemy"
      expect:
        result.entity_id: "> 0"           # 现有字段：entity_id / name / components
        result.name: "Enemy"
    ...
    - method: dsengine_editor_screenshot
      params:
        path: "${report_root}/viewport.png"
      expect:
        result.format: "png"              # 现有字段：width/height/format/encoding/data(base64)
        result.encoding: "base64"

    - method: dsengine_editor_quit            # 待新增（见 §5.2）：置退出标志，主循环下一帧返回 false
      expect:
        result.ok: true
...
```
dsengine-editor.exe --automation-mode ^
...
### 5.2 ControlServer Tool（现有 32 个 + 待新增）

> **对照代码**：编辑器已注册 **32 个 `dsengine_*` Tool**（见 `apps/editor_cpp/src/editor_control_tools.cpp` 的 dispatch 表）。用例 DSL **直接复用现有工具名**，不另造点分 taxonomy。下表区分「已有可直接用」与「确需新增」：

| Tool | 功能 | 状态 | 预估 |
|:-----|:------|:----:|:----:|
| `dsengine_ping` | 返回 `{pong:true}`，确认就绪 | ✅ 已有 | — |
| `dsengine_scene_get_state` | 返回 `entities[]`/`entity_count`/`editor_state`（即「实体列表」） | ✅ 已有 | — |
| `dsengine_entity_create` | 创建实体，返回 `entity_id`/`name`/`components` | ✅ 已有 | — |
| `dsengine_scene_save` / `dsengine_scene_load` | 保存/加载场景，返回 `saved`/`loaded`+`entity_count` | ✅ 已有 | — |
| `dsengine_editor_screenshot` | RHI RT 回读 → `width`/`height`/`format`/`encoding`/`data`(base64) | ✅ 已有 | — |
| `dsengine_editor_play` / `dsengine_editor_stop` / `dsengine_editor_undo` / `dsengine_editor_redo` | 播放/停止/撤销/重做 | ✅ 已有 | — |
| `dsengine_project_open` | 打开**工程级**（现有仅 scene 级 load，触发 SceneTab/Hierarchy/Inspector 刷新） | 🆕 待新增 | 0.5天 |
| `dsengine_editor_quit` | 置退出标志，主循环下一帧返回 false（自动化退出用） | 🆕 待新增 | 0.5天 |
| `dsengine_editor_idle` | 跑 N 帧主循环后返回当前状态（等待异步加载/渲染稳定） | 🆕 待新增 | 0.5天 |
| `dsengine_editor_get_metrics` | 返回 FPS / DrawCall / 实体数等性能数据（soak 用） | 🆕 待新增（P2） | 0.5天 |

> **单一真源（强烈建议）**：再加一个 `dsengine_list_tools` 自省工具，让用例 schema 从 C++ dispatch 表生成/校验——本设计早期版本曾出现「点分名 vs `dsengine_*`」漂移，自省可从根上杜绝。
...
| 新增 ControlServer Tool（仅 `dsengine_project_open` / `dsengine_editor_quit` 2 个 P0；其余 30 个复用） | 0.5 天 | 多数已有底层 API 可复用 |
| 编辑器无头启动（隐藏窗口） | 0.5 天 | `glfwWindowHint(GLFW_VISIBLE, GLFW_FALSE)` |
| **引擎端总计** | **3 天** | |

框架 Python 控制器（`dse_auto.py`）预估 **2-3 天**，与引擎端改造可并行进行。

> **可选简化（进一步降本）**：Batch（CLI）与 API Session 是两条独立引擎入口（两套 arg parser + 两套报告写出）。其实「无头 + 一段 YAML `calls[]`」的 API Session 已能覆盖确定性批处理流程；若把 Batch 实现为「headless 模式下的一次 API 会话」，可省掉 §5.1 的 `--automation-mode` CLI action 解析（约 2 天），引擎端降到 ~1.5 天。代价是确定性流程也得起一次 WebSocket。
    ...
    "cases": [
        {
            ...
            "duration_ms": 12000,
            "steps": [
                {"name": "dsengine_ping", "status": "passed"},
                {"name": "dsengine_project_open", "status": "passed"},
                {"name": "dsengine_scene_get_state", "status": "passed"},
                {"name": "dsengine_entity_create", "status": "passed"},
                {"name": "dsengine_scene_save", "status": "passed"},
                {"name": "dsengine_editor_screenshot", "status": "passed"},
                {"name": "dsengine_editor_quit", "status": "passed"}
            ]
        }
    ]
}
...
```powershell
$env:EDITOR_EXE = "C:\DSEngine\bin\dsengine-editor.exe"
...
.\tests\automation\scripts\run_all.ps1 `
  -EditorExe "C:\DSEngine\bin\dsengine-editor.exe" `
...
| 层级 | 内容 | 运行频率 | Runner | 预估时间 |
|:-----|:-----|:---------|:-------|:--------|
| **L0 - 提交检查** | 无头 C++ 测试（ctest unit/integration/smoke 三套件，数千断言） | 每次 PR | GitHub 托管（无需 GPU） | ~1min |
| **L1 - 日构建** | Batch 冒烟（3-5 case） | 每天 | **自托管真机 GPU** | ~30s |
| **L2 - 夜间回归** | API 全量 suite（~20+ case） | 每晚 | **自托管真机 GPU** | ~10min |
| **L3 - 稳定循环** | Stability 100 次迭代 | 每晚 | **自托管真机 GPU** | ~1h |

> **Runner 的 GPU 现实（重要）**：L1+ 要「启动编辑器 + 截图」，依赖真实 GPU。GitHub 托管 `windows-2022` runner **无独立 GPU**——编辑器的 GL 上下文要么起不来、要么落到软件光栅器（WARP/llvmpipe），截图基准对比会抖（即 `RHI_ABSTRACTION_BOUNDARY.md` §8.2 **D12** 记的软渲回归）。故 **L0 可上公有 runner，L1+ 必须自托管带 GPU 的 runner**。
...
jobs:
  editor-automation:
    runs-on: [self-hosted, windows, gpu]   # L1+ 需真实 GPU；GitHub 托管 runner 无 GPU（见 §8.1 / D12）
    steps:
      ...
      - name: Run Batch Smoke
        env:
          EDITOR_EXE: ${{ github.workspace }}/bin/dsengine-editor.exe
...
对比算法用像素级 MSE（均方误差）或 SSIM（结构相似性）。**优先复用仓库现有像素设施**：`tests/gtest/smoke/rhi_pixel_harness.h` 已有 `ComputeRmse`（RMSE 阈值判定），Python 侧用 `Pillow + numpy` 即可对齐同一套 RMSE 口径，避免再引一套对比栈（`pixelmatch` 是 npm 包、与 §A.5 的 `pillow` 不一致，不要用）。
...
| **截图** | 窗口/控件截图 | `dsengine_editor_screenshot`（RHI RT 回读） |
| **报告格式** | JSON + HTML | 完全复用 |
...
Phase 1（3-4 天）：引擎端改造
  ...
  ├── dsengine_ping / dsengine_editor_quit / dsengine_project_open Tool
  └── 无头启动（隐藏窗口）
