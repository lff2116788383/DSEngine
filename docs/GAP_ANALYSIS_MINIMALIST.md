...
### 风格化渲染：当前 ~65%（已实现 Toon/Cel Shading + Color Banding + Outline/Edge Detection）
...
✅ 已补齐：Toon/Cel Shading（`shading_mode=4`，三后端 7 参数 PerMaterial UBO）、Color Banding（IGN 抖动三后端同步）、**Outline/Edge Detection**（屏幕空间深度+法线边缘检测，三后端 GL/Vulkan/DX11 shader 同步，OutlinePass 集成 FramePipeline，Lua API `set_post_process_outline`）
