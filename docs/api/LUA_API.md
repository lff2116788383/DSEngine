...
> 更新日期：2026-06-30
> 绑定文件：`engine/scripting/lua/bindings/` 下约 29 个手写 C++ 源文件 + 13 个 `*.gen.cpp`，
> 涵盖 17 个顶层模块（其中 `http` / `net` 为条件编译）。
> 实测共注册 **905** 个 Lua 可见函数（含 §18 的 330 个 Codegen 字段访问器 + §19 的 43 个开放世界系统函数）。
