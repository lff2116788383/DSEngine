...
> 深耕：风格化渲染 + 写实渲染
>
> ⚠️ **本轮重大更新说明：** 自上次文档生成以来，引擎经历了**关键 Phase 1 收官**。完整实现了 Toon/Cel Shading DSSL 材质 + 三后端集成、Clear Coat/Anisotropy/POM 三后端参数管线、Color Banding IGN 抖动三后端同步。至此「画质细腻度 + 风格化启动」全部完成。**新增 Outline/Edge Detection 后处理 Pass**（基于深度+法线的屏幕空间边缘检测，三后端 shader 同步，Lua API 暴露）。当前引擎短板重心已从 Phase 1 转移到 Phase 2（SSBO→UBO fallback、.dds 纹理、LTCG 编译）和 Phase 3（Volumetric Fog、Decal）。
...
### 风格化渲染：当前 ~80%（已实现 Toon/Cel + Color Banding + Outline + NPR Light Model）

#### 缺失项

| 缺失技术                       |  重要性 |  难度  | 说明                                   |
| -------------------------- | :--: | :--: | ------------------------------------ |
| ~~Custom NPR Light Model~~ | 🟡 中 | 🟢 低 | ✅ DSSL light() 回调已实现 (hatching/gradient_ramp/minnaert) |

✅ 已补齐：Toon/Cel Shading（`shading_mode=4`，三后端 7 参数 PerMaterial UBO）、Color Banding（IGN 抖动三后端同步）、**Outline/Edge Detection**（屏幕空间深度+法线边缘检测，三后端 GL/Vulkan/DX11 shader 同步，OutlinePass 集成 FramePipeline，Lua API `set_post_process_outline`）、**Custom NPR Light Model**（DSSL light() 回调：hatching/gradient_ramp/minnaert 三预设）
...
```
[当前] Toon/Cel Shading + Color Banding + Outline/Edge Detection ✅
       ↓ Phase 2「引擎级支持」(1-2周)
[阶段1] + 水彩/素描风 DSSL 材质
       ↓ Phase 3「风格化材质库」(3-5天)
...
| 任务                                         |  类型 | 工作量 |
| :----------------------------------------- | :-: | :-: |
| ✅ ~~DSSL 风格化材质库（toon / outline / watercolor 等）~~ | 风格化 | ✅ 完成 |
| ✅ ~~NPR 光照模型集合（DSSL light()）~~               | 风格化 | ✅ 完成 |
