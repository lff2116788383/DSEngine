...
- 若干 2D smoke 测试标签
- 部分 3D 组件与冒烟测试入口
- `engine.3d.scene_mvp`（最小 3D MVP 场景基线门禁，对应 `assets/scenes/3d_mvp_minimal.scene.json`）
- `engine.3d.runtime_mvp_smoke`（runtime 以 `DSE_STARTUP_SCENE` 加载最小 3D MVP 场景的冒烟门禁）
- `engine.asset_compiler`（`glTF/GLB -> .dmesh/.dmat/.danim/.dskel` 的 3D 资源导入/烹饪链门禁）
- `AssetBuilder`（`apps/tools/asset_builder/main.cpp`，用于将单个 `glTF/GLB` 烹饪到上述运行时资产格式的最小工具入口）
...
```bash
cmake -S . -B build_vs2022 -G "Visual Studio 17 2022" -A x64 -DDSE_ENABLE_3D=OFF
...
```bash
cmake -S . -B build_vs2022 -G "Visual Studio 17 2022" -A x64 -DDSE_BUILD_ENGINE_TESTS=ON -DDSE_ENABLE_3D=OFF
...
- `doc-archive/DOC-00_INDEX.md`
- `doc-archive/DOC-01_ARCHITECTURE.md`
- `doc-archive/DOC-02_BUILD_AND_RUN.md`
- `doc-archive/DOC-03_EDITOR.md`
- `doc-archive/DOC-04_TESTING.md`
- `doc-archive/DOC-07_ROADMAP.md`
- `doc-archive/SUBMODULES.md`

专题文档按需阅读：

- `doc-archive/DOC-05_UI_LOCALIZATION.md`
- `doc-archive/DOC-06_PARTICLE_PROFILER.md`
...
完整文档入口见：`doc-archive/DOC-00_INDEX.md`
...
- [`tests/`](tests/)：测试与回归验证
- [`doc-archive/`](doc-archive/)：架构、构建、测试与路线图历史文档

这种分层方式是合理的，说明项目已经具备进一步工程化治理的基础。

#### 2. 文档定位与实际代码基本一致

[`README.md`](README.md) 与 [`doc-archive/DOC-01_ARCHITECTURE.md`](doc-archive/DOC-01_ARCHITECTURE.md:1) 对项目当前状态的描述比较克制，明确指出：
