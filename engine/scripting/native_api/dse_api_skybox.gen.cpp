/**
 * @file dse_api_skybox.gen.cpp
 * @brief 自动生成 — 勿手动修改
 *        来源：tools/codegen/binding_defs.json（组件：SkyboxComponent）
 *
...
/* ---- SkyboxComponent ---- */
extern "C" int dse_skybox_get_enabled(uint32_t e) {
    const auto* c = GCC<dse::SkyboxComponent>(e);
    return (c && c->enabled) ? 1 : 0;
}
extern "C" void dse_skybox_set_enabled(uint32_t e, int v) {
    if (auto* c = GC<dse::SkyboxComponent>(e)) {
        c->enabled = (v != 0);
    }
}
extern "C" void dse_skybox_set_cubemap_path(uint32_t e, const char* v) {
    if (auto* c = GC<dse::SkyboxComponent>(e)) {
        c->cubemap_path = v ? v : "";
    }
}
extern "C" int dse_skybox_get_cubemap_path(uint32_t e, char* buf, int buf_size) {
    if (!buf || buf_size <= 0) return 0;
    buf[0] = '\0';
    const auto* c = GCC<dse::SkyboxComponent>(e);
    if (!c || c->cubemap_path.empty()) return 0;
    std::strncpy(buf, c->cubemap_path.c_str(), static_cast<std::size_t>(buf_size - 1));
    buf[buf_size - 1] = '\0';
