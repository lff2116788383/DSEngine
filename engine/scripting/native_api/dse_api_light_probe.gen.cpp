/**
 * @file dse_api_light_probe.gen.cpp
 * @brief 自动生成 — 勿手动修改
 *        来源：tools/codegen/binding_defs.json（组件：LightProbeComponent）
 *
...
/* ---- LightProbeComponent ---- */
extern "C" int dse_light_probe_get_enabled(uint32_t e) {
    const auto* c = GCC<dse::LightProbeComponent>(e);
    return (c && c->enabled) ? 1 : 0;
}
extern "C" void dse_light_probe_set_enabled(uint32_t e, int v) {
    if (auto* c = GC<dse::LightProbeComponent>(e)) {
        c->enabled = (v != 0);
    }
}
extern "C" float dse_light_probe_get_influence_radius(uint32_t e) {
    const auto* c = GCC<dse::LightProbeComponent>(e);
    return c ? c->influence_radius : 0.0f;
}
extern "C" void dse_light_probe_set_influence_radius(uint32_t e, float v) {
    if (auto* c = GC<dse::LightProbeComponent>(e)) {
        c->influence_radius = v;
    }
}
extern "C" int dse_light_probe_get_show_debug(uint32_t e) {
    const auto* c = GCC<dse::LightProbeComponent>(e);
    return (c && c->show_debug) ? 1 : 0;
}
extern "C" void dse_light_probe_set_show_debug(uint32_t e, int v) {
    if (auto* c = GC<dse::LightProbeComponent>(e)) {
        c->show_debug = (v != 0);
    }
}
