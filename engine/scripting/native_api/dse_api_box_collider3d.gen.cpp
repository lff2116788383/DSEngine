/**
 * @file dse_api_box_collider3d.gen.cpp
 * @brief 自动生成 — 勿手动修改
 *        来源：tools/codegen/binding_defs.json（组件：BoxCollider3DComponent）
 *
...
extern "C" void dse_box_collider3d_set_size(uint32_t e, float x, float y, float z) {
    if (auto* c = GC<dse::BoxCollider3DComponent>(e)) {
...
extern "C" int dse_box_collider3d_get_is_trigger(uint32_t e) {
    const auto* c = GCC<dse::BoxCollider3DComponent>(e);
    return (c && c->is_trigger) ? 1 : 0;
}
extern "C" void dse_box_collider3d_set_is_trigger(uint32_t e, int v) {
    if (auto* c = GC<dse::BoxCollider3DComponent>(e)) {
        c->is_trigger = (v != 0);
    }
}
extern "C" float dse_box_collider3d_get_bounciness(uint32_t e) {
    const auto* c = GCC<dse::BoxCollider3DComponent>(e);
    return c ? c->bounciness : 0.0f;
}
extern "C" void dse_box_collider3d_set_bounciness(uint32_t e, float v) {
    if (auto* c = GC<dse::BoxCollider3DComponent>(e)) {
        c->bounciness = v;
    }
}
extern "C" float dse_box_collider3d_get_friction(uint32_t e) {
    const auto* c = GCC<dse::BoxCollider3DComponent>(e);
    return c ? c->friction : 0.0f;
}
extern "C" void dse_box_collider3d_set_friction(uint32_t e, float v) {
    if (auto* c = GC<dse::BoxCollider3DComponent>(e)) {
        c->friction = v;
    }
}
