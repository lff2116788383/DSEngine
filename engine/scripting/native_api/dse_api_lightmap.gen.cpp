/**
 * @file dse_api_lightmap.gen.cpp
 * @brief 自动生成 — 勿手动修改
 *        来源：tools/codegen/binding_defs.json（组件：LightmapComponent）
 *
...
/* ---- LightmapComponent ---- */
extern "C" void dse_lightmap_set_lightmap_path(uint32_t e, const char* v) {
    if (auto* c = GC<dse::render::LightmapComponent>(e)) {
        c->lightmap_path = v ? v : "";
    }
}
extern "C" int dse_lightmap_get_lightmap_path(uint32_t e, char* buf, int buf_size) {
    if (!buf || buf_size <= 0) return 0;
    buf[0] = '\0';
    const auto* c = GCC<dse::render::LightmapComponent>(e);
    if (!c || c->lightmap_path.empty()) return 0;
    std::strncpy(buf, c->lightmap_path.c_str(), static_cast<std::size_t>(buf_size - 1));
    buf[buf_size - 1] = '\0';
    return static_cast<int>(std::strlen(buf));
}
extern "C" float dse_lightmap_get_intensity(uint32_t e) {
    const auto* c = GCC<dse::render::LightmapComponent>(e);
    return c ? c->intensity : 0.0f;
}
extern "C" void dse_lightmap_set_intensity(uint32_t e, float v) {
    if (auto* c = GC<dse::render::LightmapComponent>(e)) {
        c->intensity = v;
    }
}
...
extern "C" void dse_lightmap_set_st_offset(uint32_t e, float x, float y, float z, float w) {
    if (auto* c = GC<dse::render::LightmapComponent>(e)) c->st_offset = glm::vec4(x, y, z, w);
}
extern "C" int dse_lightmap_get_use_ao(uint32_t e) {
    const auto* c = GCC<dse::render::LightmapComponent>(e);
    return (c && c->use_ao) ? 1 : 0;
}
extern "C" void dse_lightmap_set_use_ao(uint32_t e, int v) {
    if (auto* c = GC<dse::render::LightmapComponent>(e)) {
        c->use_ao = (v != 0);
    }
}
