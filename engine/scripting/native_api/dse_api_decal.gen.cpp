/**
 * @file dse_api_decal.gen.cpp
 * @brief 自动生成 — 勿手动修改
 *        来源：tools/codegen/binding_defs.json（组件：DecalComponent）
 *
...
/* ---- DecalComponent ---- */
extern "C" int dse_decal_get_enabled(uint32_t e) {
    const auto* c = GCC<dse::DecalComponent>(e);
    return (c && c->enabled) ? 1 : 0;
}
extern "C" void dse_decal_set_enabled(uint32_t e, int v) {
    if (auto* c = GC<dse::DecalComponent>(e)) {
        c->enabled = (v != 0);
    }
}
...
extern "C" void dse_decal_set_color(uint32_t e, float x, float y, float z, float w) {
    if (auto* c = GC<dse::DecalComponent>(e)) c->color = glm::vec4(x, y, z, w);
}
extern "C" float dse_decal_get_angle_fade(uint32_t e) {
    const auto* c = GCC<dse::DecalComponent>(e);
    return c ? c->angle_fade : 0.0f;
}
extern "C" void dse_decal_set_angle_fade(uint32_t e, float v) {
    if (auto* c = GC<dse::DecalComponent>(e)) {
        c->angle_fade = v;
    }
}
