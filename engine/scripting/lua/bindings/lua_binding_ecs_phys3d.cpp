...
int L_EcsAddRigidBody3D(lua_State* L) {
    ...
    dse_rigidbody3d_add(EID(e), type, mass);
    return 0;
}
...
int L_EcsAddBoxCollider3D(lua_State* L) {
    ...
    dse_box_collider3d_add(EID(e), x, y, z);
    return 0;
}
...
int L_EcsAddSphereCollider3D(lua_State* L) {
    ...
    dse_sphere_collider3d_add(EID(e), radius);
    return 0;
}
...
int L_EcsAddCharacterController3D(lua_State* L) {
    ...
    dse_character_controller3d_add(EID(e), radius, height, slope_limit, step_offset);
    lua_pushboolean(L, 1);
    return 1;
}
...
int L_EcsCharacterController3DIsGrounded(lua_State* L) {
    Entity e = helper::CheckEntity(L, 1);
    lua_pushboolean(L, dse_character_controller3d_is_grounded(EID(e)));
    return 1;
}
...
int L_EcsCharacterController3DGetPosition(lua_State* L) {
    ...
    dse_character_controller3d_get_position(EID(e), p);
    helper::PushVec3(L, glm::vec3(p[0], p[1], p[2]));
    return 3;
}
...
int L_EcsAddTerrainHeightmap(lua_State* L) {
    ...
    dse_terrain_heightmap_add(EID(e), ox, oz, bs, cols, rows, sc, fz ? 1 : 0);
    return 0;
}
...
int L_EcsTerrainHeightmapSetData(lua_State* L) {
    ...
    std::vector<float> heights(static_cast<size_t>(n > 0 ? n : 0));
    for (int i = 0; i < n; ++i) {
        lua_rawgeti(L, 2, i + 1);
        heights[i] = static_cast<float>(lua_tonumber(L, -1));
        lua_pop(L, 1);
    }
    dse_terrain_heightmap_set_data(EID(e), heights.empty() ? nullptr : heights.data(), n);
    return 0;
}
...
int L_EcsTerrainGetHeight(lua_State* L) {
    ...
    float y = 0.0f;
    dse_terrain_get_height(wx, wz, &y);
...
int L_EcsAddMeshCollider3D(lua_State* L) {
    ...
    dse_mesh_collider3d_add(EID(e), convex ? 1 : 0, is_trigger ? 1 : 0);
    return 0;
}
...
int L_EcsAddCapsuleCollider3D(lua_State* L) {
    ...
    dse_capsule_collider3d_add(EID(e), radius, height, direction, is_trigger ? 1 : 0);
    return 0;
}
...
int L_EcsAddJoint3D(lua_State* L) {
    ...
    dse_joint3d_add(EID(e), static_cast<uint32_t>(connected_id), type,
                    ax, ay, az, bx, by, bz, bf, bt);
    return 0;
}
...
int L_EcsSetJoint3DHingeLimits(lua_State* L) {
    Entity e = helper::CheckEntity(L, 1);
    float lower = helper::CheckFloat(L, 2);
    float upper = helper::CheckFloat(L, 3);
    dse_joint3d_set_hinge_limits(EID(e), lower, upper);
    return 0;
}
...
int L_EcsSetJoint3DSpring(lua_State* L) {
    Entity e = helper::CheckEntity(L, 1);
    float stiffness = helper::CheckFloat(L, 2);
    float damping = helper::CheckFloat(L, 3);
    dse_joint3d_set_spring(EID(e), stiffness, damping);
    return 0;
}
...
int L_EcsSetJoint3DDistance(lua_State* L) {
    ...
    dse_joint3d_set_distance(EID(e), min_d, max_d);
    return 0;
}
...
int L_EcsIsJoint3DBroken(lua_State* L) {
    Entity e = helper::CheckEntity(L, 1);
    lua_pushboolean(L, dse_joint3d_is_broken(EID(e)));
    return 1;
}
...
int L_EcsSetCollisionLayer(lua_State* L) {
    ...
    dse_collision_set_layer(EID(e), layer, mask);
    return 0;
}
...
int L_EcsSetColliderTrigger(lua_State* L) {
    ...
    dse_collider_set_trigger(EID(e), trigger ? 1 : 0);
    return 0;
}
...
int L_EcsSetColliderMaterial(lua_State* L) {
    ...
    dse_collider_set_material(EID(e), friction, bounciness);
    return 0;
}
...
int L_Physics3DOverlapSphere(lua_State* L) {
    ...
    uint32_t hits[256];
    int n = dse_physics3d_overlap_sphere(cx, cy, cz, radius, hits, 256);
    if (n > 256) n = 256;
    lua_newtable(L);
    for (int i = 0; i < n; ++i) {
        helper::PushEntity(L, static_cast<Entity>(static_cast<entt::id_type>(hits[i])));
        lua_rawseti(L, -2, i + 1);
    }
    return 1;
}
...
int L_Physics3DOverlapBox(lua_State* L) {
    ...
    uint32_t hits[256];
    int n = dse_physics3d_overlap_box(qmin.x, qmin.y, qmin.z, qmax.x, qmax.y, qmax.z, hits, 256);
    if (n > 256) n = 256;
    lua_newtable(L);
    for (int i = 0; i < n; ++i) {
        helper::PushEntity(L, static_cast<Entity>(static_cast<entt::id_type>(hits[i])));
