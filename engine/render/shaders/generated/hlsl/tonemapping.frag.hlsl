...
{
    float _68_u_manual_exposure : packoffset(c0);
    int _68_u_auto_exposure_enabled : packoffset(c0.y);
    int _68_u_lut_enabled : packoffset(c0.z);
    float _68_u_lut_intensity : packoffset(c0.w);
};

Texture2D<float4> screenTexture : register(t0);
SamplerState _screenTexture_sampler : register(s0);
Texture2D<float4> autoExposureTex : register(t1);
SamplerState _autoExposureTex_sampler : register(s1);
Texture3D<float4> u_lut : register(t2);
SamplerState _u_lut_sampler : register(s2);
...
void frag_main()
{
    ...
    float finalExposure = _68_u_manual_exposure;
    if (_68_u_auto_exposure_enabled != 0)
    {
    ...
    if (_68_u_lut_enabled != 0)
    {
        float3 lutColor = u_lut.Sample(_u_lut_sampler, clamp(result, 0.0f.xxx, 1.0f.xxx)).xyz;
        result = lerp(result, lutColor, _68_u_lut_intensity.xxx);
    }
