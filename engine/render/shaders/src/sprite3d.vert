...
// HD-2D Sprite3D billboard vertex shader.
// CPU packs world position + local quad corner + size + mode; expansion happens
// here so the same vertices can be used by OpenGL, Vulkan and D3D11.
...
void main() {
    ...
    if (mode == 0) {
        // None: use the entity transform axes (flat card in world space).
        right = normalize(aAxisX);
        up = normalize(aAxisY);
    } else if (mode == 1) {
        // Yaw: face the camera around world Y only.
        right = normalize(vec3(view[0][0], view[1][0], view[2][0]));
        up = vec3(0.0, 1.0, 0.0);
    } else if (mode == 2) {
        // YawPitch: fully view-facing.
        right = normalize(vec3(view[0][0], view[1][0], view[2][0]));
    ...
    vec4 clip;
    if (mode == 3) {
        // Screen: expand directly in clip space so the sprite keeps a fixed
        // pixel size and orientation (HUD / damage numbers).
