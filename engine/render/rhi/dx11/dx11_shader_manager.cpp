...
#include "engine/render/shaders/generated/embed/forward_shaded_morph_vert.gen.h"
...
#include "engine/render/shaders/generated/embed/forward_shaded_skinned_instanced_vert_reflect.gen.h"
...
void DX11ShaderManager::InitBuiltinShaders(std::function<void()> keep_alive) {
    ...
    // ---- 蒙皮 forward PBR 着色器 (B2b-2)：蒙皮 VS + 静态 PBR frag ----
    forward_pbr_skinned_shader_handle_ = CreateProgramFromDXBC(
        kforward_pbr_skinned_vert_dxbc, kforward_pbr_skinned_vert_dxbc_size,
        kforward_pbr_frag_dxbc, kforward_pbr_frag_dxbc_size);
    if (forward_pbr_skinned_shader_handle_) {
        DEBUG_LOG_INFO("[D3D11] Builtin forward PBR skinned shader created (DXBC): {}", forward_pbr_skinned_shader_handle_);
        using namespace generated_shaders::reflect;
        std::vector<D3D11_INPUT_ELEMENT_DESC> fpbrs_layout;
        CreateInputLayoutFromReflection(kforward_pbr_skinned_vert_reflection, fpbrs_layout);
        CreateInputLayoutForShader(forward_pbr_skinned_shader_handle_, fpbrs_layout.data(),
                                   static_cast<int>(fpbrs_layout.size()));
    }
    pulse();

    // ---- 实例化 forward PBR 着色器 (B2b-3)：实例化 VS（每实例 model SSBO\@t0）+ 静态 PBR frag ----
    forward_pbr_instanced_shader_handle_ = CreateProgramFromDXBC(
        kforward_pbr_instanced_vert_dxbc, kforward_pbr_instanced_vert_dxbc_size,
        kforward_pbr_frag_dxbc, kforward_pbr_frag_dxbc_size);
    if (forward_pbr_instanced_shader_handle_) {
        DEBUG_LOG_INFO("[D3D11] Builtin forward PBR instanced shader created (DXBC): {}", forward_pbr_instanced_shader_handle_);
        using namespace generated_shaders::reflect;
        std::vector<D3D11_INPUT_ELEMENT_DESC> fpbri_layout;
        CreateInputLayoutFromReflection(kforward_pbr_instanced_vert_reflection, fpbri_layout);
        CreateInputLayoutForShader(forward_pbr_instanced_shader_handle_, fpbri_layout.data(),
                                   static_cast<int>(fpbri_layout.size()));
    }
    pulse();
    ...
    // ---- 蒙皮 + 高级 shading 组合着色器 (Final-Feat-2)：蒙皮 VS（骨骼 SSBO\@t0）+ forward_shaded.frag ----
    forward_skinned_shaded_shader_handle_ = CreateProgramFromDXBC(
        kforward_shaded_skinned_vert_dxbc, kforward_shaded_skinned_vert_dxbc_size,
        kforward_shaded_frag_dxbc, kforward_shaded_frag_dxbc_size);
    if (forward_skinned_shaded_shader_handle_) {
        DEBUG_LOG_INFO("[D3D11] Builtin forward skinned shaded shader created (DXBC): {}", forward_skinned_shaded_shader_handle_);
        using namespace generated_shaders::reflect;
        std::vector<D3D11_INPUT_ELEMENT_DESC> fss_layout;
    ...
    // ---- 蒙皮 + 实例化 + 高级 shading 组合着色器 (阶段4-M1)：蒙皮×实例化 VS（实例 SSBO\@t0 + 骨骼 SSBO\@t1）+ forward_shaded.frag ----
    forward_skinned_instanced_shaded_shader_handle_ = CreateProgramFromDXBC(
        kforward_shaded_skinned_instanced_vert_dxbc, kforward_shaded_skinned_instanced_vert_dxbc_size,
        kforward_shaded_frag_dxbc, kforward_shaded_frag_dxbc_size);
    if (forward_skinned_instanced_shaded_shader_handle_) {
        DEBUG_LOG_INFO("[D3D11] Builtin forward skinned instanced shaded shader created (DXBC): {}", forward_skinned_instanced_shaded_shader_handle_);
        using namespace generated_shaders::reflect;
        std::vector<D3D11_INPUT_ELEMENT_DESC> fsis_layout;
        CreateInputLayoutFromReflection(kforward_shaded_skinned_instanced_vert_reflection, fsis_layout);
        CreateInputLayoutForShader(forward_skinned_instanced_shaded_shader_handle_, fsis_layout.data(),
                                   static_cast<int>(fsis_layout.size()));
    }
    pulse();

    // ---- Morph target + 高级 shading 组合着色器 (Final-Feat-5)：morph VS（增量 SSBO\@t0 + 权重 UBO\@b8）+ forward_shaded.frag ----
    forward_morph_shaded_shader_handle_ = CreateProgramFromDXBC(
        kforward_shaded_morph_vert_dxbc, kforward_shaded_morph_vert_dxbc_size,
        kforward_shaded_frag_dxbc, kforward_shaded_frag_dxbc_size);
    if (forward_morph_shaded_shader_handle_) {
        DEBUG_LOG_INFO("[D3D11] Builtin forward morph shaded shader created (DXBC): {}", forward_morph_shaded_shader_handle_);
        using namespace generated_shaders::reflect;
        std::vector<D3D11_INPUT_ELEMENT_DESC> fms_layout;
        CreateInputLayoutFromReflection(kforward_shaded_morph_vert_reflection, fms_layout);
        CreateInputLayoutForShader(forward_morph_shaded_shader_handle_, fms_layout.data(),
                                   static_cast<int>(fms_layout.size()));
    }
    pulse();
