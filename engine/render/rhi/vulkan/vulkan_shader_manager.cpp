...
void VulkanShaderManager::InitSprite2DShader() {
    using namespace dse::render::generated_shaders;
    sprite2d_shader_handle_ = CreateProgramFromSpirv(
        ksprite2d_vert_spv, ksprite2d_vert_spv_size,
        ksprite2d_frag_spv, ksprite2d_frag_spv_size);
...
void VulkanShaderManager::InitForwardPbrShader() {
    ...
    using namespace dse::render::generated_shaders;
    forward_pbr_shader_handle_ = CreateProgramFromSpirv(
...
void VulkanShaderManager::InitForwardSkinnedShadedShader() {
    if (forward_skinned_shaded_shader_handle_ != 0) return;
    using namespace dse::render::generated_shaders;
    // 蒙皮 VS（骨骼 SSBO\@set7.b0，避开 frag set0-6）+ 高级 shading frag；descriptor set layout 由 SPIR-V 反射驱动。
    forward_skinned_shaded_shader_handle_ = CreateProgramFromSpirv(
        kforward_shaded_skinned_vert_spv, kforward_shaded_skinned_vert_spv_size,
        kforward_shaded_frag_spv, kforward_shaded_frag_spv_size);
    if (forward_skinned_shaded_shader_handle_ == 0) {
        DEBUG_LOG_ERROR("Vulkan forward skinned shaded shader creation failed (pre-compiled SPIR-V)");
    } else {
        DEBUG_LOG_INFO("Vulkan forward skinned shaded shader created: handle={}", forward_skinned_shaded_shader_handle_);
    }
}

void VulkanShaderManager::InitForwardInstancedShadedShader() {
    if (forward_instanced_shaded_shader_handle_ != 0) return;
    using namespace dse::render::generated_shaders;
    // 实例化 VS（每实例 model SSBO\@set7.b0，避开 frag set0-6）+ 高级 shading frag；descriptor set layout 由 SPIR-V 反射驱动。
    forward_instanced_shaded_shader_handle_ = CreateProgramFromSpirv(
...
void VulkanShaderManager::InitForwardSkinnedInstancedShadedShader() {
    if (forward_skinned_instanced_shaded_shader_handle_ != 0) return;
    using namespace dse::render::generated_shaders;
    // 蒙皮×实例化 VS（实例 SSBO\@set8.b0 + 骨骼 SSBO\@set8.b1，避开 frag set0-6 与 set7.b1）+ 高级 shading frag；descriptor set layout 由 SPIR-V 反射驱动。
    forward_skinned_instanced_shaded_shader_handle_ = CreateProgramFromSpirv(
        kforward_shaded_skinned_instanced_vert_spv, kforward_shaded_skinned_instanced_vert_spv_size,
        kforward_shaded_frag_spv, kforward_shaded_frag_spv_size);
    if (forward_skinned_instanced_shaded_shader_handle_ == 0) {
        DEBUG_LOG_ERROR("Vulkan forward skinned instanced shaded shader creation failed (pre-compiled SPIR-V)");
    } else {
        DEBUG_LOG_INFO("Vulkan forward skinned instanced shaded shader created: handle={}", forward_skinned_instanced_shaded_shader_handle_);
    }
}
...
void VulkanShaderManager::InitForwardPbrInstancedShader() {
    if (forward_pbr_instanced_shader_handle_ != 0) return;
    using namespace dse::render::generated_shaders;
    // 实例化 VS（每实例 model SSBO\@set3.b0）+ 复用静态 PBR frag；descriptor set layout 由 SPIR-V 反射驱动。
    forward_pbr_instanced_shader_handle_ = CreateProgramFromSpirv(
        kforward_pbr_instanced_vert_spv, kforward_pbr_instanced_vert_spv_size,
        kforward_pbr_frag_spv, kforward_pbr_frag_spv_size);
...
void VulkanShaderManager::InitForwardPbrDepthShader() {
    if (forward_pbr_depth_shader_handle_ != 0) return;
    using namespace dse::render::generated_shaders;
    // 仅深度：复用静态 forward_pbr.vert + 空 shadow.frag；descriptor set layout 由 SPIR-V 反射驱动（仅 PerFrame UBO）。
    forward_pbr_depth_shader_handle_ = CreateProgramFromSpirv(
        kforward_pbr_vert_spv, kforward_pbr_vert_spv_size,
...
void VulkanShaderManager::InitTextSdfShader() {
    using namespace dse::render::generated_shaders;
    text_sdf_shader_handle_ = CreateProgramFromSpirv(
        ksprite_vert_spv, ksprite_vert_spv_size,
        ktext_sdf_frag_spv, ktext_sdf_frag_spv_size);
