...
/// 命令缓冲：把后端无关的录制接口逐调用转发到 WebGPURhiDevice 的设备级 Cmd*，
...
RenderDeviceInfo WebGPURhiDevice::GetDeviceInfo() const {
    ...
    info.is_software = false;  // 实际软/硬由浏览器适配器决定（可经 adapter info 进一步精确填充）
    return info;
}
...
void WebGPURhiDevice::Shutdown() {
    ...
    // 录制缓存 / 池 / 瞬态：管线缓存（pipeline+layout+4×BGL）、push 缓冲池、本帧 BindGroup、临时面视图。
    ...
    // compute：着色器 module、compute 管线缓存（pipeline+layout+4×BGL）。
    ...
    // SetComputeTextureImageMip 缓存的单 mip 视图统一释放。
...
void WebGPURhiDevice::EndFrame() {
    ...
    // 本帧若无任何真实绘制落到 backbuffer（无 WGSL module 的程序在录制期被优雅跳过），
    // 跑 bring-up 自检：经 Cmd* 把渐变×棋盘纹理画上屏，验证整条录制链路。
    // 任一真实内容上屏后，backbuffer_drawn_ 置真，自检自动不再触发。
...
#ifdef DSE_WEBGPU_SELFTEST
    // 每会话各一次：在本帧 frame_encoder_ 上录制全部 pending 离屏自检（外置 harness，编译期门控）。
...
// ============================================================
// 资源映射：真实 WebGPU 缓冲 / 纹理 / 采样器 / 渲染目标
// ============================================================
//
// 句柄表（buffers_/textures_/render_targets_/pipeline_states_/shaders_）把后端无关的
// unsigned int 句柄映射到原生 WGPU 对象。命令缓冲录制（依赖 WGSL 着色器与
...
// 命名 compute uniform：按调用序 16B 对齐定位（与 GL/DX11/Vulkan 同方案）。名字首见时分配下
...
void WebGPURhiDevice::DeleteTexture(unsigned int texture_handle) {
    ...
    InvalidateComputeMipViews(texture_handle);  // 先释放该句柄缓存的单 mip 视图
...
void WebGPURhiDevice::Submit(std::shared_ptr<CommandBuffer> cmd_buffer) {
    // WebGPUCommandBuffer 录制即时落到本帧 frame_encoder_（经设备级 Cmd*），故 Submit
...
// module 的程序在 IssueDraw 前优雅跳过；首帧若无任何真实绘制落屏，由 EndFrame bring-up 自检兜底。
...
// 真 indirect 绘制——按当前已绑管线/顶点/索引缓冲，从 indirect buffer（须含 Indirect
...
// MultiDrawIndexedIndirect——WebGPU 无 glMultiDrawElementsIndirect，按已绑引擎
...
// Mega VAO——WebGPU 无 VAO 对象，仅创建并记录 VBO/IBO 句柄（BatchVertex 92B 布局
...
// ============================================================
// GPU-driven PBR 着色器（手译 PBR WGSL）+ 纹理桶绑定 + 可用性查询
// ============================================================
...
// Hi-Z 遮挡剔除资源——WebGPU 无 GL 的 glTexImage2D-per-mip，改经 CreateTextureImpl 建
...
// ============================================================
// Compute 基础设施实现（compute 管线 + SSBO + indirect 原语）
// ============================================================
