 ...
 * @brief 动画状态机数据结构与逻辑
 *
 * 纯数据 + 逻辑类，零 modules/ 依赖。
 * 原路径: modules/gameplay_3d/animation/animation_state_machine.h
 * 迁移原因: engine/ecs/components_3d.h 通过 shared_ptr 持有 AnimationStateMachine，
 *           Lua 绑定、AnimatorSystem 均需使用——将定义放到 engine/ 消除反向依赖。
 */
