...
cmake -Wno-dev -Wno-deprecated -S . -B %BUILD_DIR% -G %GENERATOR% -A %ARCH% %CMAKE_EDITOR_OPTION% %CMAKE_LAUNCHER_OPTION% %CMAKE_GTEST_OPTION% %CMAKE_SPINE_OPTION% %CMAKE_BOX2D_LINKAGE_OPTION% %CMAKE_MSVC_RUNTIME_OPTION%
...
if "%BUILD_ENGINE_TESTS%"=="1" (
    ...
    rem gtest 三套经 ctest 注册为二进制级条目（dse_gtest_unit_tests 等），旧 Catch2 时代的
    rem -E 排除名（engine.lua_runtime 等）已不存在，直接全量跑 -L engine。
    ctest --test-dir %BUILD_DIR% -C %BUILD_CONFIG% --output-on-failure --verbose -L engine > "!ENGINE_TEST_LOG!" 2>&1
    set "ENGINE_TEST_RC=!ERRORLEVEL!"
    type "!ENGINE_TEST_LOG!"
    powershell -NoProfile -Command "$logPath = '!ENGINE_TEST_LOG!'; $rc = [int]'!ENGINE_TEST_RC!'; $total = 0; $passed = 0; $failed = 0; $found = $false; if (Test-Path $logPath) { $raw = Get-Content -Raw -Encoding UTF8 $logPath; $ranMatches = [regex]::Matches($raw, '(?im)^\[==========\]\s*(\d+)\s+tests? from .* ran\.'); if ($ranMatches.Count -gt 0) { $last = $ranMatches[$ranMatches.Count - 1]; $total = [int]$last.Groups[1].Value; $found = $true }; $passMatches = [regex]::Matches($raw, '(?im)^\[  PASSED  \]\s*(\d+)\s+tests?\.'); if ($passMatches.Count -gt 0) { $passed = [int]$passMatches[$passMatches.Count - 1].Groups[1].Value }; $failMatches = [regex]::Matches($raw, '(?im)^\[  FAILED  \]\s*(\d+)\s+tests?, listed below:'); if ($failMatches.Count -gt 0) { $failed = [int]$failMatches[$failMatches.Count - 1].Groups[1].Value } }; $color = if ($rc -eq 0) { 'Green' } else { 'Red' }; Write-Host ('[TEST_CASE] Total: ' + $total + ', Passed: ' + $passed + ', Failed: ' + $failed) -ForegroundColor $color; if (-not $found) { Write-Host '[WARN] GoogleTest summary not found in log. TEST_CASE stats may be incomplete.' -ForegroundColor Yellow }"
    if "!ENGINE_TEST_RC!"=="0" (
        ...
        powershell -NoProfile -Command "$logPath = '!ENGINE_TEST_LOG!'; if (Test-Path $logPath) { $raw = Get-Content -Raw -Encoding UTF8 $logPath; $pattern = '(?im)^\[  FAILED  \]\s*([^\r\n\(]+)'; $matches = [regex]::Matches($raw, $pattern); if ($matches.Count -gt 0) { Write-Host '[FAIL] Failed test cases:' -ForegroundColor Red; $seen = New-Object 'System.Collections.Generic.HashSet[string]'; foreach ($m in $matches) { $item = '- ' + $m.Groups[1].Value.Trim(); if ($seen.Add($item)) { Write-Host $item -ForegroundColor Red } } } else { Write-Host '[WARN] No failed testcase name matched in CTest log.' -ForegroundColor Yellow } } else { Write-Host '[WARN] CTest log file not found for failure parsing.' -ForegroundColor Yellow }"
        popd
        pause
