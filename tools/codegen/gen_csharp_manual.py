...
# 从 dse_api.h 提取 DSE_API_VERSION 宏
def extract_api_version(header_text: str) -> int:
    m = re.search(r'#define\s+DSE_API_VERSION\s+(\d+)u?', header_text)
    return int(m.group(1)) if m else 0

RET_MAP = {"void": "void", "int": "int", "float": "float", "uint32_t": "uint",
           "double": "double", "long long": "long", "int64_t": "long", "uint64_t": "ulong"}

# Function pointer typedef -> C# delegate mapping
# Parsed from dse_api.h typedefs
DELEGATE_MAP = {
    "dse_cutscene_camera_fn": {
        "cs_name": "DseCutsceneCameraFn",
        "params": "float px, float py, float pz, float lx, float ly, float lz, float fov, nint userData",
    },
    "dse_cutscene_event_fn": {
        "cs_name": "DseCutsceneEventFn",
        "params": "nint eventName, nint payload, nint userData",
    },
    "dse_cutscene_finish_fn": {
        "cs_name": "DseCutsceneFinishFn",
        "params": "nint seqName, nint userData",
    },
    "dse_ai_condition_fn": {
        "cs_name": "DseAiConditionFn",
        "params": "nint userData",
        "ret": "int",
    },
    "dse_ai_action_fn": {
        "cs_name": "DseAiActionFn",
        "params": "float dt, nint userData",
        "ret": "int",
    },
    "dse_ai_destroy_fn": {
        "cs_name": "DseAiDestroyFn",
        "params": "nint userData",
    },
}
...
def map_param(ctype: str, name: str, func: str):
    ...
    if ctype == "const char* const*":
        ...
        cs = f"nint {n}"
    elif ctype in DELEGATE_MAP:
        cs = f"{DELEGATE_MAP[ctype]['cs_name']} {n}"
    elif ctype.endswith("*"):
...
def main():
    ...
    # Insert delegate declarations before the Native class
    for dl in delegate_lines:
        lines.append(dl)
    lines.append("")
    lines.extend([
        "internal static partial class Native {",
        f"    /// <summary>编译时 C ABI 版本号（与 dse_api.h 中 DSE_API_VERSION 同步）</summary>",
        f"    internal const uint DSE_API_VERSION = {api_version}u;",
        "",
        f"    /// <summary>运行时版本检查：DLL 返回值 != DSE_API_VERSION 则 ABI 不兼容</summary>",
        f"    internal static bool IsApiCompatible() => dse_api_version() == DSE_API_VERSION;",
        "",
    ])
    count = 0
    ...
    for ret, name, args in decls:
        if name in SKIP or name in existing:
            continue
        has_fn = re.search(r"\b([A-Za-z0-9_]+_fn)\b", args)
        has_unknown_fn = False
        if has_fn:
            fn_types = re.findall(r"\b([A-Za-z0-9_]+_fn)\b", args)
            for ft in fn_types:
                if ft not in DELEGATE_MAP:
                    has_unknown_fn = True
                    break
        if has_unknown_fn:
            # Unknown callback type, skip
            continue
        try:
            params = []
            calls = []
